(async function(){
    let a=new Audio("https://raw.githubusercontent.com/DeanHawley/Random-Stuff/refs/heads/main/hover.mp3");a.loop=true;a.play();
    let b=false;
    document.getElementById("bigCookie").addEventListener("click",function(){
        if(b)return;
        b=true;
        a.pause();
        let c=new Audio("https://raw.githubusercontent.com/DeanHawley/Random-Stuff/refs/heads/main/click.mp3");
        c.loop=true;
        c.play();
    });
    ...
    setInterval(function(){
        var supportSection=document.getElementById('supportSection');if(supportSection)supportSection.remove();
        var warningSections=document.querySelectorAll('.listing.warning');if(warningSections.length>1)warningSections[1].remove();
        var buffs=document.getElementById('buffs');if(buffs)buffs.style.marginTop='20px';
        var ad=document.getElementById('support');if(ad)ad.remove();
        var shadow=document.querySelector('#store').previousElementSibling;if(shadow)shadow.remove();
        ...
        if(linksBar){
            Array.from(linksBar.children).forEach(function(child){if(child.id!=='heralds'){child.remove();}});
        }
        ...
        if(heralds){
            ...
            heralds.style.boxShadow='none';
            heralds.style.transform='translateX(-3px)';
        }
    },1);
    async function updateHeralds(){
        try{
            const res=await fetch('https://api.allorigins.win/get?url='+encodeURIComponent('https://api.steampowered.com/ISteamUserStats/GetNumberOfCurrentPlayers/v1/?appid=1454400'));
            const wrapper=await res.json();
            const data=JSON.parse(wrapper.contents);
            const players=data.response.player_count;
            Game.heralds=Math.min(100,Math.floor(players/100));
            l('heraldsAmount').textContent=Game.heralds;
        }catch{}
    }
    updateHeralds();
    setInterval(updateHeralds,10000);
    Game.externalDataLoaded=true;
    Game.recalculateGains=1;
    const interval=setInterval(()=>{
        const heraldsAmount=document.getElementById('heraldsAmount');
        if(heraldsAmount){
            heraldsAmount.style.top=parseInt(window.getComputedStyle(heraldsAmount).top)-4+'px';
